...
public class GermplasmQueryTest {
	...
	public void setUp() throws Exception {
		...
		Mockito.when(this.germplasmDataManager.getNamesByGID(Mockito.anyInt(), Mockito.isNull(Integer.class), Mockito.isNull(GermplasmNameType.class)))
				.thenReturn(GermplasmTestDataInitializer.createNameList(germplasms.size()));

		Mockito.when(this.germplasmDataManager.getMethodByID(Mockito.anyInt())).thenReturn(new MethodTestDataInitializer().createMethod(1,"testMethodType"));
	...
	public void testGetGermplasmItem() throws Exception {
		...
		Assert.assertTrue("the formed item object should contain the property ids", item.getItemPropertyIds().containsAll(itemPropertyIDList) && itemPropertyIDList.containsAll(item.getItemPropertyIds()));
	...
	public void testGetGermplasmSearchResults() throws Exception {
		Assert.assertEquals("Verify the call should return the expected list of germplasms", germplasms,this.query.getGermplasmSearchResults(1,20));
	}
