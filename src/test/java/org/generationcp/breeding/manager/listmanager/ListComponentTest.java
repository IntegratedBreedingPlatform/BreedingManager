...
public class ListComponentTest {
	...
	@Before
	public void setUp() throws IllegalAccessException, MiddlewareQueryException {
		...
		ListComponent _listComponent = new ListComponent(source, parentListDetailsComponent,
				germplasmList);
		FieldUtils.writeDeclaredField(_listComponent, "contextUtil", contextUtil, true);

		listComponent = spy(_listComponent);
		...
		when(source.getWindow()).thenReturn(mock(Window.class));
		when(source.getListSelectionComponent()).thenReturn(mock(ListSelectionComponent.class));
	...
	public void testSaveList_OverwriteExistingGermplasmList() {
		...
		try {
			doReturn(germplasmList).when(germplasmListManager).getGermplasmListById(
	...
	public void testSaveList_OverwriteExistingGermplasmListWithDifferentID() {
		...
		try {
			...
			doReturn(germplasmListToBeSaved).when(germplasmListManager).getGermplasmListById(
					anyInt());
	...
	public void testSaveList_OverwriteNonExistingGermplasmList() {
		...
		try {
			doReturn(null).when(germplasmListManager).getGermplasmListById(
					germplasmList.getId().intValue());
	...
	public void testLockGermplasmList() {
		...
		try {
			doReturn(germplasmList).when(germplasmListManager).getGermplasmListById(
					germplasmList.getId());
			when(source.lockGermplasmList(germplasmList)).thenReturn(true);
			germplasmList.setStatus(germplasmList.getStatus() + 100);
		} catch (MiddlewareQueryException e) {
			fail(e.getMessage());
		}

		doNothing().when(listComponent).setLockedState(anyBoolean());
	...
	public void testAddListeners() {
		ViewListHeaderComponent viewListHeaderComponent = Mockito.mock(
				ViewListHeaderComponent.class);
		...
		doNothing().when(listComponent)
				.addListenerForUpdatingSelectedEntriesInListDataAndInventoryTable();

		when(messageSource.getMessage(Message.ADD_ENTRIES)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.COPY_TO_NEW_LIST)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.DELETE_LIST)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.DELETE_SELECTED_ENTRIES)).thenReturn(
				DUMMY_OPTION);
		when(messageSource.getMessage(Message.EDIT_LIST)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.EXPORT_LIST)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.EXPORT_LIST_FOR_GENOTYPING_ORDER)).thenReturn(
				DUMMY_OPTION);
		when(messageSource.getMessage(Message.INVENTORY_VIEW)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.SAVE_CHANGES)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.SELECT_ALL)).thenReturn(DUMMY_OPTION);

		when(messageSource.getMessage(Message.CANCEL_RESERVATIONS)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.COPY_TO_NEW_LIST)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.RESERVE_INVENTORY)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.RETURN_TO_LIST_VIEW)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.SAVE_RESERVATIONS)).thenReturn(DUMMY_OPTION);

		when(messageSource.getMessage(Message.EDIT_VALUE)).thenReturn(DUMMY_OPTION);
		when(messageSource.getMessage(Message.ADD_SELECTED_ENTRIES_TO_NEW_LIST)).thenReturn(
				DUMMY_OPTION);
		...
		Mockito.verify(listComponent, Mockito.times(1))
				.addListenerForUpdatingSelectedEntriesInListDataAndInventoryTable();
	}

	@Test
	public void TestIsANonEditableColumnInListDataTable() {
		Assert.assertTrue("GID column must be a non editable column.",
				listComponent.isANonEditableColumnInListDataTable("gid"));
		Assert.assertTrue("ENTRY_ID column must be a non editable column.",
				listComponent.isANonEditableColumnInListDataTable("entryId"));
		Assert.assertTrue("DESIGNATION column must be a non editable column.",
				listComponent.isANonEditableColumnInListDataTable("desig"));
		Assert.assertFalse("PARENTAGE column must be an editable column.",
				listComponent.isANonEditableColumnInListDataTable("parentage"));
	}

	@Test
	public void TestIsAnInventoryRelatedColumn() {
		Assert.assertTrue("SEED_RESERVATION column is an inventory column.",
				listComponent.isAnInventoryRelatedColumn("seedRes"));
		Assert.assertTrue("AVAIL_INV column is an inventory column.",
				listComponent.isAnInventoryRelatedColumn("availInv"));
		Assert.assertFalse("PARENTAGE column is not an inventory column.",
				listComponent.isAnInventoryRelatedColumn("parentage"));
	}
}
