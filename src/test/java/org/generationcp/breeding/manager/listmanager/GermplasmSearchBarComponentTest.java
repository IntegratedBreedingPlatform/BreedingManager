...
public class GermplasmSearchBarComponentTest {
	...
	public void setUp() throws BreedingManagerSearchException {
		...
		Mockito.doNothing().when(this.germplasmSearchResultsComponent).applyGermplasmResults(
				Matchers.any(GermplasmSearchParameter.class));
	}
	...
	public void testDoSearchUsingStartsWithKeywordSearchType() throws Exception {
		...
		ArgumentCaptor<GermplasmSearchParameter> searchArgument = ArgumentCaptor.forClass(GermplasmSearchParameter.class);
		Mockito.verify(this.germplasmSearchResultsComponent, Mockito.times(1)).applyGermplasmResults(searchArgument.capture());
		Assert.assertEquals(TEST_SEARCH_STRING + "%", searchArgument.getValue().getSearchKeyword());
