...
public class CrossingManagerExporter {
	...
	private User listExporter;

	public CrossingManagerExporter(GermplasmList crossesList,
			List<GermplasmListData> listEntries,
			Map<Integer, CrossParents> parentsInfo) {
		super();
	...
	private void initializeUserValues() throws CrossingManagerExporterException {
		try {
			listCreator = userDataManager.getUserById(crossesList.getUserId());
		} catch (MiddlewareQueryException ex) {
			throw new CrossingManagerExporterException("Error getting owner of list.");
		}

		try {
			listExporter = userDataManager.getUserById(contextUtil.getCurrentUserLocalId());
		} catch (MiddlewareQueryException ex) {
			throw new CrossingManagerExporterException("Error getting current user.");
		}

	}

	private void initializeConditionsAndFactors() {
		conditionsToWriteOnFile = new ArrayList<ImportedCondition>();
		ImportedCondition userNameCondition = new ImportedCondition("LIST USER",
				"PERSON WHO MADE THE LIST", "PERSON", "DBCV"
				, "ASSIGNED", "C", listCreator.getName(), "LIST");
		conditionsToWriteOnFile.add(userNameCondition);
		ImportedCondition userIdCondition = new ImportedCondition("LIST USER ID",
				"ID OF LIST OWNER", "PERSON", "DBID"
				, "ASSIGNED", "N", listCreator.getUserid().toString(), "LIST");
		conditionsToWriteOnFile.add(userIdCondition);
		ImportedCondition exporterNameCondition = new ImportedCondition("LIST EXPORTER",
				"PERSON EXPORTING THE LIST", "PERSON", "DBCV"
				, "ASSIGNED", "C", listExporter.getName(), "LIST");
		conditionsToWriteOnFile.add(exporterNameCondition);
		ImportedCondition exporterIdCondition = new ImportedCondition("LIST EXPORTER ID",
				"ID OF LIST EXPORTER", "PERSON", "DBID"
				, "ASSIGNED", "N", listExporter.getUserid().toString(), "LIST");
		conditionsToWriteOnFile.add(exporterIdCondition);

		factorsToWriteOnFile = new ArrayList<ImportedFactor>();
		ImportedFactor entryIdFactor = new ImportedFactor("ENTRY", "The germplasm entry number",
				"GERMPLASM ENTRY", "NUMBER", "ENUMERATED", "N", "Entry ID");
		factorsToWriteOnFile.add(entryIdFactor);
		ImportedFactor gidFactor = new ImportedFactor("GID", "The GID of the germplasm",
				"GERMPLASM ID", "DBID", "ASSIGNED", "N", "Entry ID");
		factorsToWriteOnFile.add(gidFactor);
		ImportedFactor entryCodeFactor = new ImportedFactor("ENTRY CODE", "Germplasm entry code",
				"GERMPLASM ENTRY", "CODE", "ASSIGNED", "C", "Entry ID");
		factorsToWriteOnFile.add(entryCodeFactor);
		ImportedFactor designationFactor = new ImportedFactor("DESIGNATION",
				"The name of the germplasm", "GERMPLASM ID", "DBCV", "ASSIGNED", "C", "Entry ID");
		factorsToWriteOnFile.add(designationFactor);
		ImportedFactor crossFactor = new ImportedFactor("CROSS",
				"The pedigree string of the germplasm", "CROSS NAME", "NAME", "ASSIGNED", "C",
				"Entry ID");
		factorsToWriteOnFile.add(crossFactor);
		ImportedFactor femaleFactor = new ImportedFactor("FEMALE", "NAME OF FEMALE PARENT",
				"GERMPLASM ID", "DBCV", "FEMALE SELECTED", "C", "Entry ID");
		factorsToWriteOnFile.add(femaleFactor);
		ImportedFactor maleFactor = new ImportedFactor("MALE", "NAME OF MALE PARENT",
				"GERMPLASM ID", "DBCV", "MALE SELECTED", "C", "Entry ID");
		factorsToWriteOnFile.add(maleFactor);
		ImportedFactor femaleGIDFactor = new ImportedFactor("FEMALE GID", "GID OF FEMALE PARENT",
				"GERMPLASM ID", "DBID", "FEMALE SELECTED", "N", "Entry ID");
		factorsToWriteOnFile.add(femaleGIDFactor);
		ImportedFactor maleGIDFactor = new ImportedFactor("MALE GID", "GID OF MALE PARENT",
				"GERMPLASM ID", "DBID", "MALE SELECTED", "N", "Entry ID");
		factorsToWriteOnFile.add(maleGIDFactor);
		ImportedFactor sourceFactor = new ImportedFactor("SOURCE",
				"The seed source of the germplasm", "SEED SOURCE", "NAME", "Seed Source", "C",
				"Entry ID");
		factorsToWriteOnFile.add(sourceFactor);
	}

	public FileOutputStream exportCrossingManagerExcel(String filename)
			throws CrossingManagerExporterException {
		initializeUserValues();
		initializeConditionsAndFactors();
		...
		lastRow = writeFactorSection(sheetStyles, descriptionSheet,
				lastRow + 2); //two rows before section
	...
	private int writeListDetailsSection(HashMap<String, CellStyle> styles,
	...
	private int writeFactorSection(HashMap<String, CellStyle> styles, HSSFSheet descriptionSheet,
			int startingRow) {
		int actualRow = startingRow - 1;
		int currentRow = actualRow;
	...
	private int writeObservationsSheet(HashMap<String, CellStyle> styles,
			HSSFSheet descriptionSheet, int startingRow) {
		int actualRow = startingRow - 1;
		int currentRow = actualRow;
		...
		for (ExportCrossesObservationSheetHeaders header : ExportCrossesObservationSheetHeaders
				.values()) {
			Cell headerCell = conditionDetailsHeading.createCell(columnCtr);
		...
		currentRow++;
		for (GermplasmListData cross : listEntries) {
			HSSFRow conditionRow = descriptionSheet.createRow(currentRow++);
			...
			Integer listDataId = cross.getId();
			CrossParents parents = parentsInfo.get(listDataId);
			if (parents != null) {
				GermplasmListEntry femaleParent = parents.getFemaleParent();
				GermplasmListEntry maleParent = parents.getMaleParent();

				conditionRow.createCell(6).setCellValue(femaleParent.getDesignation());
				conditionRow.createCell(7).setCellValue(maleParent.getDesignation());
				conditionRow.createCell(8).setCellValue(femaleParent.getGid());
				conditionRow.createCell(9).setCellValue(maleParent.getGid());
			}
