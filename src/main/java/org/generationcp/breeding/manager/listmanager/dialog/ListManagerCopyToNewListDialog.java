...
public class ListManagerCopyToNewListDialog extends VerticalLayout
	...
	public void saveGermplasmListButtonClickAction()
			throws InternationalizableException, NumberFormatException {
			...
			if (listNameValue.trim().length() == 0) {
				...
				if (!existingListSelected) {
					...
					try {
						...
						try {
							...
							MessageNotifier.showError(getWindow().getParent().getWindow(),
									"Error with copying list entries."
									, "Copying of entries to a new list failed. " + messageSource
					...
					try {
						...
						//TODO must accommodate the expanding of the folder up to the parent of the list being opened
						listManagerMain.getListSelectionComponent().getListTreeComponent()
								.expandNode(ListSelectorComponent.LISTS);
						//TODO must accommodate opening in the search screen also
						listManagerMain.getListSelectionComponent().getListDetailsLayout()
								.removeTab(Integer.valueOf(listId));
						listManagerMain.getListSelectionComponent().getListTreeComponent()
	...
	private void addGermplasmListData(GermplasmList germList, int entryid)
		...
		for (final Object itemId : selectedIds) {
			Property pParentage = listEntriesTable.getItem(itemId)
					.getItemProperty(ListDataTablePropertyID.PARENTAGE.getName());
			Property pEntryId = listEntriesTable.getItem(itemId)
					.getItemProperty(ListDataTablePropertyID.ENTRY_ID.getName());
			Property pGid = listEntriesTable.getItem(itemId)
					.getItemProperty(ListDataTablePropertyID.GID.getName());
			Property pDesignation = listEntriesTable.getItem(itemId)
