...
public class ListBuilderComponent extends VerticalLayout implements InitializingBean,
	...
	static final Action[] GERMPLASMS_TABLE_CONTEXT_MENU = new Action[] { ACTION_SELECT_ALL,
			ACTION_DELETE_SELECTED_ENTRIES };
	...
	public void instantiateComponents() {
		...
		buildNewListDesc
				.setValue("Build or revise your list by dragging in entries from the left.");
		...
		if (currentlySavedGermplasmList != null) {
			...
			viewHeaderButton
					.setDescription(viewListHeaderWindow.getListHeaderComponent().toString());
		}

		breedingManagerListDetailsComponent = new BreedingManagerListDetailsComponent();

		tableWithSelectAllLayout = new TableWithSelectAllLayout(
				ListDataTablePropertyID.TAG.getName());

		if (currentlySavedGermplasmList != null) {
			listInventoryTable = new ListManagerInventoryTable(source,
					currentlySavedGermplasmList.getId(), false, true);
		} else {
		...
		menuCopyToList = menu
				.addItem(messageSource.getMessage(Message.COPY_TO_NEW_LIST_WINDOW_LABEL));
		menuDeleteSelectedEntries = menu
		...
		dropHandler = new BuildNewListDropHandler(source, germplasmDataManager,
				germplasmListManager, inventoryDataManager, tableWithSelectAllLayout.getTable());
	...
	public void addListeners() {
		...
		fillWith = new FillWith(this, messageSource, tableWithSelectAllLayout.getTable(),
			...
			public void contextItemClick(ClickEvent event) {
				...
				if (clickedItem.getName().equals(messageSource.getMessage(Message.SELECT_ALL))) {
					germplasmsTable.setValue(germplasmsTable.getItemIds());
						...
						.equals(messageSource.getMessage(Message.DELETE_SELECTED_ENTRIES))) {
					deleteSelectedEntries();
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.EXPORT_LIST))) {
					exportListAction();
				} else if (clickedItem.getName().equals(messageSource
						.getMessage(Message.EXPORT_LIST_FOR_GENOTYPING_ORDER))) {
					exportListForGenotypingOrderAction();
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.COPY_TO_NEW_LIST_WINDOW_LABEL))) {
					copyToNewListAction();
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.INVENTORY_VIEW))) {
					viewInventoryAction();
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.RESET_LIST))) {
					resetButton.click();
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.SAVE_LIST))) {
					saveButton.click();
				}
			}
		...
		inventoryViewMenu.addListener(new ContextMenu.ClickListener() {
			...
			public void contextItemClick(ClickEvent event) {
				...
				if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.RETURN_TO_LIST_VIEW))) {
					viewListAction();
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.COPY_TO_NEW_LIST))) {
					copyToNewListFromInventoryViewAction();
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.RESERVE_INVENTORY))) {
					reserveInventoryAction();
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.SELECT_ALL))) {
					listInventoryTable.getTable()
							.setValue(listInventoryTable.getTable().getItemIds());
				} else if (clickedItem.getName()
						.equals(messageSource.getMessage(Message.CANCEL_RESERVATIONS))) {
		...
		saveListButtonListener = new SaveListButtonClickListener(this,
		...
		lockButton.addListener(new ClickListener() {
			...
			public void buttonClick(com.vaadin.ui.Button.ClickEvent event) {
				if (!currentlySavedGermplasmList.isLockedList()) {
					currentlySavedGermplasmList
							.setStatus(currentlySavedGermplasmList.getStatus() + 100);
					try {
						currentlySetGermplasmInfo = currentlySavedGermplasmList;
						saveListButtonListener.doSaveAction(false);

						contextUtil.logProgramActivity("Locked a germplasm list.",
								"Locked list " + currentlySavedGermplasmList.getId() + " - "
										+ currentlySavedGermplasmList.getName());

					} catch (MiddlewareQueryException e) {
						LOG.error("Error with unlocking list.", e);
						MessageNotifier.showError(getWindow(), "Database Error!",
								"Error with loocking list. " + messageSource
				...
				if (currentlySavedGermplasmList.isLockedList()) {
					...
					try {
						...
						contextUtil.logProgramActivity("Unlocked a germplasm list.",
								"Unlocked list " + currentlySavedGermplasmList.getId() + " - "
										+ currentlySavedGermplasmList.getName());
	...
	public void layoutComponents() {
		...
		final HeaderLabelLayout headingLayout = new HeaderLabelLayout(
				AppConstants.Icons.ICON_BUILD_NEW_LIST, buildNewListTitle);
		...
		listBuilderPanel.setStyleName(
				Reindeer.PANEL_LIGHT + " " + AppConstants.CssStyles.PANEL_GRAY_BACKGROUND);
		...
		HeaderLabelLayout listEntriesTitle = new HeaderLabelLayout(
				AppConstants.Icons.ICON_LIST_TYPES, topLabel);
		...
		listBuilderPanelTitleContainer
				.setComponentAlignment(viewHeaderButton, Alignment.BOTTOM_RIGHT);
		listBuilderPanelTitleContainer
				.setComponentAlignment(editHeaderButton, Alignment.BOTTOM_RIGHT);
	...
	private void addBasicTableColumns(Table table) {
		...
		table.addContainerProperty(ListDataTablePropertyID.DESIGNATION.getName(), Button.class,
				null);
		table.addContainerProperty(ListDataTablePropertyID.PARENTAGE.getName(), String.class, null);
		table.addContainerProperty(ListDataTablePropertyID.AVAILABLE_INVENTORY.getName(),
				Button.class, null);
		table.addContainerProperty(ListDataTablePropertyID.SEED_RESERVATION.getName(), String.class,
				null);
		table.addContainerProperty(ListDataTablePropertyID.ENTRY_CODE.getName(), String.class,
				null);
		table.addContainerProperty(ListDataTablePropertyID.GID.getName(), Button.class, null);
		table.addContainerProperty(ListDataTablePropertyID.SEED_SOURCE.getName(), String.class,
				null);
	...
	private void deleteSelectedEntries() {
		Collection<? extends Integer> selectedIdsToDelete = (Collection<? extends Integer>) tableWithSelectAllLayout
				.getTable().getValue();
		if (!selectedIdsToDelete.isEmpty()) {
			if (listDataTable.getItemIds().size() == selectedIdsToDelete.size()) {
				...
				for (Integer selectedItemId : selectedIdsToDelete) {
					tableWithSelectAllLayout.getTable().getContainerDataSource()
	...
	private void assignSerializedEntryNumber() {
		...
		for (Integer itemId : itemIds) {
			tableWithSelectAllLayout.getTable().getItem(itemId)
					.getItemProperty(ListDataTablePropertyID.ENTRY_ID.getName()).setValue(id);
			id++;
		}
	}
	...
	public List<GermplasmListData> getListEntriesFromTable() {
		...
		for (Object id : this.tableWithSelectAllLayout.getTable().getItemIds()) {
			...
			Button designationButton = (Button) item
			...
			Object entryCode = item.getItemProperty(ListDataTablePropertyID.ENTRY_CODE.getName())
					.getValue();
			if (entryCode != null) {
			...
			Object groupName = item.getItemProperty(ListDataTablePropertyID.PARENTAGE.getName())
					.getValue();
			if (groupName != null) {
			...
			listEntry.setEntryId(
					(Integer) item.getItemProperty(ListDataTablePropertyID.ENTRY_ID.getName())
							.getValue());

			Object seedSource = item.getItemProperty(ListDataTablePropertyID.SEED_SOURCE.getName())
					.getValue();
			if (seedSource != null) {
	...
	private void exportListAction() {
		ExportListAsDialog exportListAsDialog = new ExportListAsDialog(source,
				currentlySavedGermplasmList, listDataTable);
	...
	private void exportListForGenotypingOrderAction() throws InternationalizableException {
		if (isCurrentListSaved()) {
			if (currentlySavedGermplasmList.isLockedList()) {
				...
				GermplasmListExporter listExporter = new GermplasmListExporter(
				...
				try {
					...
					fileDownloadResource
							.setFilename(listName.replace(" ", "_") + "ForGenotyping.xls");
	...
	public void copyToNewListAction() {
		if (isCurrentListSaved()) {
			Collection<?> listEntries = (Collection<?>) tableWithSelectAllLayout.getTable()
					.getValue();

			if (listEntries == null || listEntries.isEmpty()) {
				MessageNotifier.showRequiredFieldError(source.getWindow(),
						messageSource.getMessage(Message.ERROR_LIST_ENTRIES_MUST_BE_SELECTED));
			} else {
	...
	public void openSaveListAsDialog() {
		dialog = new SaveListAsDialog(this, currentlySavedGermplasmList,
		...
		if (!inventoryViewMenu.isVisible()) {
			...
			if (hasUnsavedChanges()) {
				...
				if (lotDetailsGid == null || lotDetailsGid.isEmpty()) {
					MessageNotifier
							.showError(getWindow(), messageSource.getMessage(Message.WARNING),
	...
	public void saveReservationChangesAction() {
		...
		if (hasUnsavedChanges()) {
			...
			List<ListDataAndLotDetails> listDataAndLotDetails = listInventoryTable
			...
			for (ListDataAndLotDetails listDataAndLotDetail : listDataAndLotDetails) {
				if (!alreadyAddedEntryIds.contains(listDataAndLotDetail.getEntryId())) {
					dropHandler.addGermplasmFromList(listDataAndLotDetail.getListId(),
			...
			for (ListDataAndLotDetails listDataAndLotDetail : listDataAndLotDetails) {
				listInventoryTable.getInventoryTableDropHandler()
						.assignLrecIdToRowsFromListWithEntryId(listDataAndLotDetail.getListId(),
								listDataAndLotDetail.getEntryId());
			}
			...
			boolean success = reserveInventoryAction
					.saveReserveTransactions(getValidReservationsToSave(),
							currentlySavedGermplasmList.getId());
			if (success) {
	...
	public void cancelReservations() {
		...
		listInventoryTable.resetRowsForCancelledReservation(lotDetailsGid,
				currentlySavedGermplasmList.getId());
	...
	private void refreshInventoryColumns(Set<Integer> entryIds) {
		...
		try {
			if (!entryIds.isEmpty()) {
				germplasmListDataEntries = this.inventoryDataManager
						.getLotCountsForListEntries(currentlySavedGermplasmList.getId(),
		...
		for (GermplasmListData listData : germplasmListDataEntries) {
			...
			Button inventoryButton = new Button(availInv,
					new InventoryLinkButtonClickListener(source,
							currentlySavedGermplasmList.getId(), listData.getId(),
							listData.getGid()));
	...
	public void updateListInventoryTable(
		...
		for (Map.Entry<ListEntryLotDetails, Double> entry : validReservations.entrySet()) {
			...
			itemToUpdate.getItemProperty(ListManagerInventoryTable.NEWLY_RESERVED_COLUMN_ID)
					.setValue(newRes);
		}
