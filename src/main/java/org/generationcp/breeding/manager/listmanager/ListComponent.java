...
import org.generationcp.breeding.manager.inventory.*;
...
public class ListComponent extends VerticalLayout implements InitializingBean,
	...
	public void instantiateComponents() {
		...
		instantiateViewListHeaderWindow();
		...
		try {
			listEntriesCount = germplasmListManager
					.countGermplasmListDataByListId(germplasmList.getId());
		} catch (MiddlewareQueryException ex) {
			LOG.error("Error with retrieving count of list entries for list: " + germplasmList
					.getId(), ex);
			listEntriesCount = 0;
		}
		...
		if (listEntriesCount == 0) {
			totalListEntriesLabel
					.setValue(messageSource.getMessage(Message.NO_LISTDATA_RETRIEVED_LABEL));
		} else {
		...
		menuExportForGenotypingOrder = menu
				.addItem(messageSource.getMessage(Message.EXPORT_LIST_FOR_GENOTYPING_ORDER));
		...
		menuCancelReservation = inventoryViewMenu
				.addItem(messageSource.getMessage(Message.CANCEL_RESERVATIONS));
		menuCopyToNewListFromInventory = inventoryViewMenu
				.addItem(messageSource.getMessage(Message.COPY_TO_NEW_LIST));
		menuReserveInventory = inventoryViewMenu
				.addItem(messageSource.getMessage(Message.RESERVE_INVENTORY));
		menuListView = inventoryViewMenu
				.addItem(messageSource.getMessage(Message.RETURN_TO_LIST_VIEW));
		menuInventorySaveChanges = inventoryViewMenu
				.addItem(messageSource.getMessage(Message.SAVE_RESERVATIONS));
		...
		tableContextMenuSelectAll = tableContextMenu
				.addItem(messageSource.getMessage(Message.SELECT_ALL));
		tableContextMenuDeleteEntries = tableContextMenu
				.addItem(messageSource.getMessage(Message.DELETE_SELECTED_ENTRIES));
		tableContextMenuEditCell = tableContextMenu
				.addItem(messageSource.getMessage(Message.EDIT_VALUE));
		tableContextMenuCopyToNewList = tableContextMenu
				.addItem(messageSource.getMessage(Message.ADD_SELECTED_ENTRIES_TO_NEW_LIST));
		...
		updateListStatusWhenLaunchFromDashboard();
	}

	protected void updateListStatusWhenLaunchFromDashboard() {
	...
	protected void instantiateViewListHeaderWindow() {
	...
	protected void initializeListDataTable() {
		listDataTableWithSelectAll = new TableWithSelectAllLayout(
				Long.valueOf(listEntriesCount).intValue(), getNoOfEntries(),
				ListDataTablePropertyID.TAG.getName());
		...
		listDataTable
				.addContainerProperty(ListDataTablePropertyID.TAG.getName(), CheckBox.class, null);
		listDataTable
				.addContainerProperty(ListDataTablePropertyID.ENTRY_ID.getName(), Integer.class,
						null);
		listDataTable
				.addContainerProperty(ListDataTablePropertyID.DESIGNATION.getName(), Button.class,
						null);
		listDataTable
				.addContainerProperty(ListDataTablePropertyID.PARENTAGE.getName(), String.class,
						null);
		listDataTable.addContainerProperty(ListDataTablePropertyID.AVAILABLE_INVENTORY.getName(),
				Button.class, null);
		listDataTable.addContainerProperty(ListDataTablePropertyID.SEED_RESERVATION.getName(),
				String.class, null);
		listDataTable
				.addContainerProperty(ListDataTablePropertyID.ENTRY_CODE.getName(), String.class,
						null);
		listDataTable
				.addContainerProperty(ListDataTablePropertyID.GID.getName(), Button.class, null);
		listDataTable
				.addContainerProperty(ListDataTablePropertyID.SEED_SOURCE.getName(), String.class,
						null);

		messageSource.setColumnHeader(listDataTable, ListDataTablePropertyID.TAG.getName(),
				ListDataTablePropertyID.TAG.getColumnDisplay());
		messageSource.setColumnHeader(listDataTable, ListDataTablePropertyID.ENTRY_ID.getName(),
				ListDataTablePropertyID.ENTRY_ID.getColumnDisplay());
		messageSource.setColumnHeader(listDataTable, ListDataTablePropertyID.DESIGNATION.getName(),
				ListDataTablePropertyID.DESIGNATION.getColumnDisplay());
		messageSource.setColumnHeader(listDataTable, ListDataTablePropertyID.PARENTAGE.getName(),
				ListDataTablePropertyID.PARENTAGE.getColumnDisplay());
		messageSource.setColumnHeader(listDataTable,
				ListDataTablePropertyID.AVAILABLE_INVENTORY.getName(),
				ListDataTablePropertyID.AVAILABLE_INVENTORY.getColumnDisplay());
		messageSource
				.setColumnHeader(listDataTable, ListDataTablePropertyID.SEED_RESERVATION.getName(),
						ListDataTablePropertyID.SEED_RESERVATION.getColumnDisplay());
		messageSource.setColumnHeader(listDataTable, ListDataTablePropertyID.ENTRY_CODE.getName(),
				ListDataTablePropertyID.ENTRY_CODE.getColumnDisplay());
		messageSource.setColumnHeader(listDataTable, ListDataTablePropertyID.GID.getName(),
				ListDataTablePropertyID.GID.getColumnDisplay());
		messageSource.setColumnHeader(listDataTable, ListDataTablePropertyID.SEED_SOURCE.getName(),
	...
	public void loadEntriesToListDataTable() {
		if (listEntriesCount > 0) {
			...
			listDataTable.sort(new Object[] { ListDataTablePropertyID.ENTRY_ID.getName() },
					new boolean[] { true });
			...
			try {
				...
				LOG.error(
						"Error with displaying added columns for entries of list: " + germplasmList
								.getId(), ex);
			}
	...
	private void addListEntryToTable(GermplasmListData entry) {
		...
		Button desigButton = new Button(entry.getDesignation(),
				new GidLinkButtonClickListener(source, gid, true, true));
		...
		newItem.getItemProperty(ListDataTablePropertyID.ENTRY_ID.getName())
				.setValue(entry.getEntryId());
		newItem.getItemProperty(ListDataTablePropertyID.DESIGNATION.getName())
				.setValue(desigButton);
		newItem.getItemProperty(ListDataTablePropertyID.PARENTAGE.getName())
				.setValue(entry.getGroupName());
		newItem.getItemProperty(ListDataTablePropertyID.ENTRY_CODE.getName())
				.setValue(entry.getEntryCode());
		newItem.getItemProperty(ListDataTablePropertyID.GID.getName()).setValue(gidButton);
		newItem.getItemProperty(ListDataTablePropertyID.SEED_SOURCE.getName())
		...
		Button inventoryButton = new Button(availInv,
				new InventoryLinkButtonClickListener(parentListDetailsComponent,
						germplasmList.getId(), entry.getId(), entry.getGid()));
		inventoryButton.setStyleName(BaseTheme.BUTTON_LINK);
		inventoryButton.setDescription(CLICK_TO_VIEW_INVENTORY_DETAILS);
		newItem.getItemProperty(ListDataTablePropertyID.AVAILABLE_INVENTORY.getName())
		...
		newItem.getItemProperty(ListDataTablePropertyID.SEED_RESERVATION.getName())
				.setValue(seedRes);
	}
	...
	private void getAllListEntries() {
		...
		try {
			entries = inventoryDataManager.getLotCountsForList(germplasmList.getId(), 0,
					Long.valueOf(listEntriesCount).intValue());
	...
	public void addListeners() {
		...
		addListenerToActionMenuInListView();

		addListenerToActionMenuInInventoryView();
		...
		addListenerToLockUnlockButton();

		addListenerToListDataTableContextMenu();

		addListenerForUpdatingSelectedEntriesInListDataAndInventoryTable();

	}
	//end of addListeners

	protected void addListenerForUpdatingSelectedEntriesInListDataAndInventoryTable() {
	...
	protected void addFillWithForUnLockedList() {
		if (!germplasmList.isLockedList()) {
			fillWith = new FillWith(parentListDetailsComponent, parentListDetailsComponent,
					messageSource, listDataTable, ListDataTablePropertyID.GID.getName());
		}
	}

	protected void addListenerToLockUnlockButton() {
		lockButton.addListener(new ClickListener() {
			private static final long serialVersionUID = 1L;

			@Override
			public void buttonClick(com.vaadin.ui.Button.ClickEvent event) {
				lockGermplasmList(germplasmList.getId());
			}
		});
	...
	protected void addListenerToListDataTableContextMenu() {
		tableContextMenu.addListener(new ContextMenu.ClickListener() {
			...
			public void contextItemClick(ClickEvent event) {
				...
				if (action.equals(messageSource.getMessage(Message.DELETE_SELECTED_ENTRIES))) {
					...
					performEditValueInListDataTable();
	...
	private void performEditValueInListDataTable() {
	...
	protected void addListenerToActionMenuInInventoryView() {
		inventoryViewMenu.addListener(new ContextMenu.ClickListener() {
			...
			public void contextItemClick(ClickEvent event) {
				...
				if (clickedItem.getName().equals(messageSource.getMessage(Message.SELECT_ALL))) {
					listInventoryTable.getTable()
							.setValue(listInventoryTable.getTable().getItemIds());
				} else {
					doSelectedActionItemInInventoryView(clickedItem.getName());
				}
			}
		});
	}

	protected void doSelectedActionItemInInventoryView(String clickedItem) {
		if (clickedItem.equals(messageSource.getMessage(Message.SAVE_RESERVATIONS))) {
			saveReservationChangesAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.RETURN_TO_LIST_VIEW))) {
			viewListAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.COPY_TO_NEW_LIST))) {
			copyToNewListFromInventoryViewAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.RESERVE_INVENTORY))) {
			reserveInventoryAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.CANCEL_RESERVATIONS))) {
			cancelReservationsAction();
		}
	}

	protected void addListenerToActionMenuInListView() {
		menu.addListener(new ContextMenu.ClickListener() {
			...
			public void contextItemClick(ClickEvent event) {
				...
				if (clickedItem.getName().equals(messageSource.getMessage(Message.SELECT_ALL))) {
					...
					doSelectedActionItemInListView(clickedItem.getName());
				}
			}
		});
	}

	protected void doSelectedActionItemInListView(String clickedItem) {
		if (clickedItem.equals(messageSource.getMessage(Message.EXPORT_LIST))) {
			exportListAction();
		} else if (clickedItem
				.equals(messageSource.getMessage(Message.EXPORT_LIST_FOR_GENOTYPING_ORDER))) {
			exportListForGenotypingOrderAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.COPY_TO_NEW_LIST))) {
			copyToNewListAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.ADD_ENTRIES))) {
			addEntryButtonClickAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.SAVE_CHANGES))) {
			saveChangesAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.DELETE_SELECTED_ENTRIES))) {
			deleteEntriesButtonClickAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.EDIT_LIST))) {
			editListButtonClickAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.DELETE_LIST))) {
			deleteListButtonClickAction();
		} else if (clickedItem.equals(messageSource.getMessage(Message.INVENTORY_VIEW))) {
			viewInventoryAction();
		}
	}
		...
		listDataTable.addListener(new ItemClickListener() {
			...
			public void itemClick(ItemClickEvent event) {
				...
				if (event.getButton() == ItemClickEvent.BUTTON_RIGHT) {
					...
					if (selectedColumn.equals(ListDataTablePropertyID.TAG.getName())
							|| isANonEditableColumnInListDataTable(selectedColumn)
							|| isAnInventoryRelatedColumn(selectedColumn)) {
				...
				tf.addListener(new BlurListener() {
					...
					public void blur(BlurEvent event) {
						...
						for (Map.Entry<Object, Field> entry : itemMap.entrySet()) {
							...
							if (column.equals(selectedColumn) && selectedColumn
									.equals(ListDataTablePropertyID.DESIGNATION.getName())) {
								...
								if (isDesignationValid(designation, gid)) {
									...
									ConfirmDialog.show(getWindow(), "Update Designation",
											...
											, "Yes", "No", new ConfirmDialog.Listener() {
												...
												public void onClose(ConfirmDialog dialog) {
													if (!dialog.isConfirmed()) {
														...
														Double d = computeTextFieldWidth(
	...
	protected boolean isANonEditableColumnInListDataTable(Object selectedColumn) {
		return selectedColumn.equals(ListDataTablePropertyID.GID.getName())
	...
	protected boolean isAnInventoryRelatedColumn(Object selectedColumn) {
		return selectedColumn.equals(ListDataTablePropertyID.SEED_RESERVATION.getName())
	...
	public boolean isDesignationValid(String designation, int gid) {
		...
		try {
			...
			MessageNotifier
					.showError(getWindow(), DATABASE_ERROR, "Error with validating designation."
			...
			if (listDataTable.size() == selectedIdsToDelete.size()) {
				ConfirmDialog.show(this.getWindow(),
						...
						new ConfirmDialog.Listener() {
							...
							public void onClose(ConfirmDialog dialog) {
								if (dialog.isConfirmed()) {
									removeRowsInListDataTable(
											(Collection<?>) listDataTable.getValue());
								}
							}
		...
		try {
			...
			if (!gidsWithoutChildren.isEmpty()) {
				ConfirmDialog.show(this.getWindow(), "Delete Germplasm from Database",
						"Would you like to delete the germplasm(s) from the database also?",
						"Yes", "No", new ConfirmDialog.Listener() {
							private static final long serialVersionUID = 1L;
		...
		if (listDataTable.getItemIds().size() == selectedIds.size()) {
			...
			for (final Object itemId : selectedIds) {
				Button desigButton = (Button) listDataTable.getItem(itemId)
						.getItemProperty(ListDataTablePropertyID.DESIGNATION.getName()).getValue();
	...
	private List<Integer> getGidsToDeletedWithoutChildren(Collection<?> selectedIds)
		...
		for (final Object itemId : selectedIds) {
			Button gidButton = (Button) listDataTable.getItem(itemId)
					.getItemProperty(ListDataTablePropertyID.GID.getName()).getValue();
		...
		if (germplasmList.isLockedList()) {
			...
			try {
				...
				FileDownloadResource fileDownloadResource = new FileDownloadResource(
						new File(tempFileName), source.getApplication());
	...
	public Boolean finishAddingEntry(Integer gid, Boolean showSuccessMessage) {
		...
		try {
			...
			MessageNotifier.showError(getWindow(), DATABASE_ERROR,
		...
		try {
			...
			if (listDataTable == null) {
				...
				if (isColumnVisible(visibleColumns, AddColumnContextMenu.CROSS_FEMALE_GID)) {
					addColumnContextMenu.setCrossFemaleInfoColumnValues(false,
							AddColumnContextMenu.CROSS_FEMALE_GID);
				}
				if (isColumnVisible(visibleColumns, AddColumnContextMenu.CROSS_FEMALE_PREF_NAME)) {
					addColumnContextMenu.setCrossFemaleInfoColumnValues(false,
							AddColumnContextMenu.CROSS_FEMALE_PREF_NAME);
				}
			...
			MessageNotifier.showError(getWindow(), DATABASE_ERROR,
					"Error with adding list entry. " + messageSource
							.getMessage(Message.ERROR_REPORT_TO));
			return false;
		}
	...
	public Boolean saveChangesAction(Window window, Boolean showSuccessMessage) {
		...
		try {
			listEntriesCount = this.germplasmListManager
					.countGermplasmListDataByListId(germplasmList.getId());
			listEntries = this.germplasmListManager
					.getGermplasmListDataByListId(germplasmList.getId(), 0, (int) listEntriesCount);
		} catch (MiddlewareQueryException e) {
		...
		for (Iterator<?> i = listDataTable.getItemIds().iterator(); i.hasNext(); ) {
			...
			for (GermplasmListData listData : listEntries) {
				if (listData.getId().equals(listDataId)) {
					...
					String entryCode = (String) item
							.getItemProperty(ListDataTablePropertyID.ENTRY_CODE.getName())
							.getValue();
					...
					String seedSource = (String) item
							.getItemProperty(ListDataTablePropertyID.SEED_SOURCE.getName())
							.getValue();
					...
					Button desigButton = (Button) (item
							.getItemProperty(ListDataTablePropertyID.DESIGNATION.getName()))
							.getValue();
					String designation = String.valueOf(desigButton.getCaption().toString());
					...
					String parentage = (String) item
							.getItemProperty(ListDataTablePropertyID.PARENTAGE.getName())
							.getValue();
		...
		try {
			...
			germplasmListManager.saveListDataColumns(
					addColumnContextMenu.getListDataCollectionFromTable(listDataTable));
	...
	private void performListEntriesDeletion(Map<Object, String> itemsToDelete) {
		try {
			...
			designationOfListEntriesDeleted = designationOfListEntriesDeleted
					.substring(0, designationOfListEntriesDeleted.length() - 1);

			//Change entry IDs on listData
			List<GermplasmListData> listDatas = germplasmListManager
					.getGermplasmListDataByListId(germplasmList.getId(), 0
							, (int) germplasmListManager
							.countGermplasmListDataByListId(germplasmList.getId()));
			Integer entryId = 1;
		...
		if (!germplasmList.isLockedList()) {
			try {
				ListCommonActionsUtil.deleteGermplasmList(germplasmListManager,
						germplasmList, contextUtil, getWindow(), messageSource, "list");
	...
	public void lockGermplasmList(Integer listId) {
		try {
	...
	public void unlockGermplasmList() {
		if (germplasmList.isLockedList()) {
			...
			try {
				...
				contextUtil.logProgramActivity("Unlocked a germplasm list.",
						"Unlocked list " + germplasmList.getId() + " - " + germplasmList.getName());

			} catch (MiddlewareQueryException e) {
				LOG.error("Error with unlocking list.", e);
				MessageNotifier.showError(getWindow(), DATABASE_ERROR,
						"Error with unlocking list. " + messageSource
								.getMessage(Message.ERROR_REPORT_TO));
			}
		}
	}
		...
		if (savedList != null) {
			if (!savedList.getId().equals(germplasmList.getId())) {
				...
				if (viewHeaderButton != null) {
					viewHeaderButton.setDescription(
							viewListHeaderWindow.getListHeaderComponent().toString());
				}
			}
		}
	...
	private void updateNoOfEntries(long count) {
		...
		if (source.getModeView().equals(ModeView.LIST_VIEW)) {
			if (count == 0) {
				...
				totalListEntriesLabel.setValue(
						messageSource.getMessage(Message.TOTAL_LIST_ENTRIES) + ": " + countLabel);
			}
		} else {
			//Inventory View
			totalListEntriesLabel
	...
	private void updateNoOfSelectedEntries() {
		...
		if (source.getModeView().equals(ModeView.LIST_VIEW)) {
			Collection<?> selectedItems = (Collection<?>) listDataTableWithSelectAll.getTable()
					.getValue();
	...
	public void saveReservationChangesAction() {
		if (hasUnsavedChanges()) {
			...
			boolean success = reserveInventoryAction
					.saveReserveTransactions(getValidReservationsToSave(), germplasmList.getId());
			if (success) {
	...
	public void cancelReservationsAction() {
		...
		if (lotDetailsGid == null || lotDetailsGid.isEmpty()) {
			...
			if (!listInventoryTable.isSelectedEntriesHasReservation(lotDetailsGid)) {
				...
				ConfirmDialog
						.show(getWindow(), messageSource.getMessage(Message.CANCEL_RESERVATIONS),
	...
	private void refreshInventoryColumns(Set<Integer> entryIds) {
		...
		try {
			if (!entryIds.isEmpty()) {
				germplasmListDataEntries = this.inventoryDataManager
						.getLotCountsForListEntries(germplasmList.getId(),
		...
		for (GermplasmListData listData : germplasmListDataEntries) {
			...
			Button inventoryButton = new Button(availInv,
					new InventoryLinkButtonClickListener(parentListDetailsComponent,
							germplasmList.getId(), listData.getId(), listData.getGid()));
			inventoryButton.setStyleName(BaseTheme.BUTTON_LINK);
			inventoryButton.setDescription(CLICK_TO_VIEW_INVENTORY_DETAILS);
			...
			Button gidButton = (Button) item.getItemProperty(ListDataTablePropertyID.GID.getName())
					.getValue();
			String gidString = "";
	...
	public void updateListInventoryTable(
		...
		for (Map.Entry<ListEntryLotDetails, Double> entry : validReservations.entrySet()) {
			...
			itemToUpdate.getItemProperty(ListInventoryTable.NEWLY_RESERVED_COLUMN_ID)
					.setValue(newRes);
		}
	...
	private void updateAvailInvValues(Integer gid, String availInv) {
		...
		for (Integer itemId : itemIds) {
			...
			if (currentGid.equals(gid)) {
				((Button) item
						.getItemProperty(ListDataTablePropertyID.AVAILABLE_INVENTORY.getName())
						.getValue()).setCaption(availInv);
			}
		}
		listDataTable.requestRepaint();
	}
