...
public class RemoveSelectedGermplasmAsDialog extends BaseSubWindow
	...
	public void instantiateComponents() {
		...
		this.titleRemoveGermplasmLabel = new Label(this.messageSource.getMessage(Message.REMOVE_SELECTED_GERMPLASM).toUpperCase());
		...
		this.warningTextRemoveGermplasmLabel = new Label(this.messageSource.getMessage(Message.WARNING_REMOVE_SELECTED_GERMPLASM));
		this.warningTextRemoveGermplasmLabel.setDebugId("warningTextRemoveGermplasmLabel");

		this.cancelButton = new Button(this.messageSource.getMessage(Message.NO));
		this.cancelButton.setDebugId("cancelButton");
		this.cancelButton.setWidth("80px");

		this.acceptButton = new Button(this.messageSource.getMessage(Message.YES));
	...
	public void layoutComponents() {
		// window formatting
		this.setCaption(this.messageSource.getMessage(Message.REMOVE_SELECTED_GERMPLASM));
		this.center();
	...
	static class AcceptButtonListener implements Button.ClickListener {
		...
		public void buttonClick(final Button.ClickEvent event) {
			...
			if (false) {
				message = "20 germplasm were deteled successfully!";
				MessageNotifier.showMessage(this.removeSelectedGermplasmAsDialog.getParent().getWindow(), null, message);
			} else if (false) {
				message =
					"15 germplasm were deteled successfully and 5 can not be deleted due to internal validations (has inventory, it is used in other lists or it is fixed). For further information please contact support.";
				MessageNotifier.showWarning(this.removeSelectedGermplasmAsDialog.getParent().getWindow(), "Warning!", message);
			} else if (true) {
				message =
					"5 germplasm can not be deleted due to internal validations (has inventory, it is used in other lists or it is fixed). For further information please contact support.";
