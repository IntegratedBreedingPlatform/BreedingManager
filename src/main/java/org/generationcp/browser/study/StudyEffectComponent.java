...
public class StudyEffectComponent extends I18NVerticalLayout{
    ...
    public StudyEffectComponent(StudyDataManager studyDataManager, int studyId, Accordion accordion, I18N i18n) {
        super(i18n);
            ...
            for (Representation rep : representations) {
                if (rep.getName() != null) {
                    if (!rep.getName().equals(i18n.getMessage("studyEffect.header"))) { // "STUDY EFFECT"
                        datasets.add(i18n.getMessage("dataset.text") + " " + rep.getId() + " - " + rep.getName()); // Dataset
                    }
                } else {
                    datasets.add(i18n.getMessage("dataset.text") + " " + rep.getId() + " - " + rep.getName()); // Dataset
                }
    ...
    public void datasetListValueChangeAction(String datasetLabel) {
        ...
        Integer repId = Integer.valueOf(parts[0].replaceAll(getI18N().getMessage("dataset.text"), "").trim()); // "Dataset"
