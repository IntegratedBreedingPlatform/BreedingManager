...
public class SearchGermplasmByPhenotypicTab extends GridLayout{
    ...
    public SearchGermplasmByPhenotypicTab(GidByPhenotypicQueries gidsByPhenotypicParam,
	    TraitDataIndexContainer dataIndexContainerParam) throws QueryException {
	...
	// set column headers
	criteriaTable.setColumnHeaders(new String[] { "TraitID", "ScaleID", "MethodID", "Trait Name", "Scale Name",
		"Method Name", "Value", "Scale Datatype", "Scale Discrete Value" });
	criteriaTable.setVisibleColumns(new Object[] { "traitName", "scaleName", "methodName", "criteriaValue",
		"scaleDiscreteValue" });
	...
	int methodID = Integer.valueOf(traitMethodTable.getItem(traitMethodTable.getValue())
		.getItemProperty("methodID").toString());
	...
	for (Iterator i = criteriaTable.getItemIds().iterator(); i.hasNext();) {
	    ...
	    if (scaleType.equals("discrete")) {
		String valueToUse = (scaleDiscreteValue != null && !scaleDiscreteValue.equals("")) ? scaleDiscreteValue
			: criteriaValue;
		...
		try {
		    ...
		    TraitCombinationFilter filter = new TraitCombinationFilter(new Integer(traitID), new Integer(
			    scaleID), new Integer(methodID), valueToUseInDouble);
		...
		if (criteriaValue.contains("-")) {
			...
			try {
			    ...
			    TraitCombinationFilter tcFilter = new TraitCombinationFilter(new Integer(traitID),
				    new Integer(scaleID), new Integer(methodID), ranges);
			    tcf.add(tcFilter);
			} catch (NumberFormatException ex) {
		...
		if (notNumericRange) {
		    TraitCombinationFilter tcFilter = new TraitCombinationFilter(new Integer(traitID), new Integer(
			    scaleID), new Integer(methodID), criteriaValue);
		    tcf.add(tcFilter);
		}
	    }
