...
/**
 * Results are displayed here. Main goal is to award points for traits that fall within a desired, specified range. 
 * Points are removed if over many locations, the traits do not meet the desired range. 
 * 
 * -- If the selected range covers all measurements for all locations, then the score will be 1.0
 * -- If there are more observations outside the range than inside, the score will be negative
 * -- Positive scores of any kind reflect more than 50% compliance in the range
 * 
 * FIXME : the numbers displayed here are not correct and must be reviewed
 * FIXME : this is a long class and further guidance through the code is required
 * 
 * @author rebecca
 *
 */
...
public class TraitDisplayResults extends AbsoluteLayout implements InitializingBean, InternationalizableComponent {
	...
	public void afterPropertiesSet() throws Exception {
		...
		this.germplasmColTable.setWidth("340px");
		this.germplasmColTable.setHeight("445px");
		...
		this.traitsColTable.setHeight("445px");
		...
		this.combinedScoreTagColTable.setHeight("445px");
	...
	public void populateRowsResultsTable(Table resultTable, Integer NoOfColumns) {
		int line_no = this.currentLineIndex + 1;
		...
		for (TableResultRow row : this.tableRows.subList(this.currentLineIndex, endOfListIndex)) {
			...
			for (Map.Entry<NumericTraitFilter, TraitObservationScore> numericTOS : numericTOSMap.entrySet()) {
				...
				int index = this.columnHeaders.indexOf("DisplayResults " + name + traitId + " numeric");
			...
			for (Map.Entry<CharacterTraitFilter, TraitObservationScore> characterTOS : characterTOSMap.entrySet()) {
				...
				int index = this.columnHeaders.indexOf("DisplayResults " + name + traitId + " character");
			...
			for (Map.Entry<CategoricalTraitFilter, TraitObservationScore> categoricalTOS : categoricalTOSMap.entrySet()) {
				...
				int index = this.columnHeaders.indexOf("DisplayResults " + name + traitId + " categorical");
	...
	public List<TableResultRow> getTableRowsResults() {
		...
		try {
			...
			List<Observation> observations = this.crossStudyDataManager.getObservationsForTraits(this.traitIds, this.environmentIds);
			this.observationsMap = this.getObservationsMap(observations);
	...
	public boolean testNumericTraitVal(NumericTraitFilter trait, Observation obsevation) {
		NumericTraitEvaluator eval =
				new NumericTraitEvaluator(trait.getCondition(), trait.getLimits(), Double.valueOf(obsevation.getValue()));
	...
	public boolean testCategoricalTraitVal(CategoricalTraitFilter trait, Observation obsevation) {
		CategoricalTraitEvaluator eval = new CategoricalTraitEvaluator(trait.getCondition(), trait.getLimits(), obsevation.getValue());
	...
	public Map<Integer, String> getGermplasm(List<Integer> traitIds, List<Integer> environmentIds) {
		...
		try {
			...
			germplasmIdNameMap = this.germplasmDataManager.getPreferredNamesByGids(germplasmIds);
	...
	public void toggleSaveButton() {
		if (this.selectedGermplasmMap.size() > 0) {
			this.saveButton.setEnabled(true);
		} else if (this.selectedGermplasmMap.size() == 0) {
			this.saveButton.setEnabled(false);
		}
	}
	...
	private void addTagAllCheckBoxToCombinedScoreTagColTable() {
		...
		this.tagAllCheckBoxOnCombinedScoreTagColTable.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				if ((Boolean) TraitDisplayResults.this.tagAllCheckBoxOnCombinedScoreTagColTable.getValue() == true) {
