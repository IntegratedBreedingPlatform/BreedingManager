 ...
 * 
 * Progression is:
 * -- Welcome (TraitWelcomeScreen)
 * -- Select traits of interest (PreselectTraitFilter)
 * -- Select and weight environments where the traits have been observed (EnvironmentFilter)
 * -- Weight and range select measurements from the trait measurements (SetUpTraitDonorFilter)
 * -- Analyse results where germplasm has performed within a desired range for as many locations as possible (TraitDisplayResults)
 * 
 * @author rebecca
 *
 */
...
public class TraitDonorsQueryMain extends VerticalLayout implements InitializingBean, InternationalizableComponent {
	...
	public void afterPropertiesSet() throws Exception {
		...
		this.setTitleContent(TraitDonorsQueryMain.GUIDE_MESSAGE);
	...
	private void setTitleContent(String guideMessage) {
		...
		String title = "Trait Donors Query  <h2>" + TraitDonorsQueryMain.VERSION + "</h2>";
		this.mainTitle = new Label();
		this.mainTitle.setStyleName(Bootstrap.Typography.H1.styleName());
		this.mainTitle.setWidth("285px");
